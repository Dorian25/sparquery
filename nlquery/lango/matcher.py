...
def match_rules(tree, rules, fun=None, matched_rule=[], multi=False):
    ...
    if multi:
        context = match_rules_context_multi(tree, rules, matched_rule)
    else:
        context = match_rules_context(tree, rules, matched_rule)
        if not context:
            return None
...
def match_rules_context(tree, rules, matched_rule, parent_context={}):
    ...
    for template, match_rules in rules.items():
        ...
        if match_template(tree, template, context):
            matched_rule.append(template)
            for key, child_rules in match_rules.items():
                child_context = match_rules_context(context[key], child_rules, matched_rule, context)
                if child_context:
